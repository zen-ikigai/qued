...
export const PATCH = async (req, { params }) => {
  ...
  try {
    ...
    if (!task || task.creator.toString() !== userId) {
      console.error(
        'Task not found or unauthorized update attempt by user:',
        userId,
      )
      return new Response(
    ...
    if (!title || !description || !status) {
      console.error(
        'Validation error: Missing required fields for task ID:',
        params.id,
      )
      return new Response(
        JSON.stringify({
          ...
          message:
            'Missing required fields: title, description, and/or status.',
        }),
        {
          status: 400,
    ...
    const updatedTask = await Task.findByIdAndUpdate(params.id, taskDetails, {
      new: true,
    })
