const TaskCard = ({ task }) => {
  ...
  return (
    <div className=" border-b border-black p-4 text-black ">
      ...
      <div>
        <p className="text-sm text-gray-600 mt-3"><b>Due By: </b>{formatDate(task.dueDate)}</p>
        <p className="text-sm text-gray-600 mt-2"><b>Created on: </b>{formatDate(task.createdAt)}</p>
        <p className="text-sm text-gray-600 mt-2"><b>Reminder: </b>{task.reminder ? 'Set' : 'Not Set'}</p>
      </div>
    </div>
  );
};
