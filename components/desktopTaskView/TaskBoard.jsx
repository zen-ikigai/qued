...
const DroppableArea = ({ id, message }) => {
  ...
  return (
    <div ref={setNodeRef} className="w-full flex items-center justify-center min-h-full text-base text-gray-600 italic">
      <p>{message}</p>
    </div>
  );
};
...
const TaskBoard = ({ tasks, setTasks, userId, fetchTasks, handleDelete, handleEdit, handleStatusChange }) => {
  const [containers, setContainers] = useState([
    { id: 'todo', status: 'todo', name: 'To Do', bgColor:'bg-red-100', titleColor: 'bg-red-300', message:'No Tasks To Do!', tasks: [] },
    { id: 'inProgress', status: 'inProgress', name: 'In Progress', bgColor:'bg-yellow-100', titleColor: 'bg-yellow-300', message:'No Tasks in Progress!', tasks: [] },
